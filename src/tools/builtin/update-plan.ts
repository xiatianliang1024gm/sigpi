...
export function createUpdatePlanTool(
	...
	return {
		name: "update_plan",
		description:
			"Track progress for multi-step tasks. Use it for any task with several dependent steps, ambiguity, or visible checkpoints. Call it at the start with the full ordered list of steps, keep exactly one step in_progress while work remains, and update it as you finish each step. Keep steps short and actionable.",
...
function isPlanItem(value: unknown): value is { step: string; status: string } {
	return (
		typeof value === "object" &&
		value !== null &&
