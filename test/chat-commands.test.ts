...
test("/history swaps the TUI children to show the history view", async () => {
	const outputs: string[] = [];
	const terminal = new FakeTerminal();
	const tui = new TUI(terminal);
	const baseChild = new BaseTextComponent(["BASE_CONTENT"]);
	tui.addChild(baseChild);

	const commandPromise = executeChatCommand(
		"/history",
		createChatCommandDefinitions(),
		{
			getState: () =>
				({
					runtime: {
						turn: {
							getCurrentSession: () => createHistorySession(2),
						},
					},
					view: { getTuiInstance: () => tui },
				}) as never,
			setState: () => {},
			store: {} as never,
			writeLine: (line: string) => outputs.push(line),
	...
	// The history view is rendered, not the plain output stream.
	assert.deepEqual(outputs, []);
	assert.ok(
		lines.some((line) => line.includes("Session history")),
		"history header is rendered",
	);
	assert.ok(
		lines.some((line) => line.includes("Turn 1 [completed]")),
		"first turn is rendered in the history view",
	);
	assert.ok(
		lines.some((line) => line.includes("Turn 2 [completed]")),
		"second turn is rendered in the history view",
	);
	assert.equal(
		lines.some((line) => line.includes("BASE_CONTENT")),
		false,
		"base content is hidden while the history view is shown",
	);
	assert.equal(
		tui.children.includes(baseChild),
		false,
		"base child is removed from tui.children while history is shown",
	);

	// Esc dismisses the history view and the command resolves.
	terminal.inputHandler?.("\x1B");
	await commandPromise;

	// After dismissal the original children are restored.
	assert.equal(tui.hasOverlay(), false, "no overlay is left behind");
	assert.equal(
		tui.children.includes(baseChild),
		true,
		"base child is restored to tui.children after dismissal",
	);

	// Let the post-dismissal render flush through.
	await new Promise((resolve) => setTimeout(resolve, 50));
	const restoredLines = renderedLines(terminal.writes);
	assert.ok(
		restoredLines.some((line) => line.includes("BASE_CONTENT")),
		"base content is visible again after dismissal",
	);
});

test("/history overlay also dismisses on Ctrl+C", async () => {
	const terminal = new FakeTerminal();
	const tui = new TUI(terminal);
	...
	assert.equal(tui.hasOverlay(), false, "overlay is dismissed after Ctrl+C");
});

test("/history overlay scrolls when the history overflows the viewport", async () => {
	const terminal = new FakeTerminal();
	terminal.columns = 80;
	terminal.rows = 10;
	const tui = new TUI(terminal);
	...
	const initialLines = renderedLines(terminal.writes);
	assert.ok(
		initialLines.some((line) => line.includes("Turn 6 [completed]")),
		"latest turn is visible at the bottom of the initial viewport",
	);
	assert.equal(
		initialLines.some((line) => line.includes("Turn 1 [completed]")),
		false,
		"earliest turn is scrolled out of the initial viewport",
	);

	// Scroll up to reveal earlier turns.
	terminal.inputHandler?.("g");
	await new Promise((resolve) => setTimeout(resolve, 50));

	const scrolledLines = renderedLines(terminal.writes);
	assert.ok(
		scrolledLines.some((line) => line.includes("Turn 1 [completed]")),
		"scrolling to the top reveals the earliest turn",
	);
...
test("/history overlay truncates lines wider than the terminal", async () => {
	const terminal = new FakeTerminal();
	terminal.columns = 40;
	terminal.rows = 10;
	const tui = new TUI(terminal);
	...
	const longInput =
		"修改/history的实现，不再使用OverlayLines的方式显示，而是先将tui中现有的children暂存起来，将HistoryOverlayComponent添加到tui中，当用户按Esc退出后，再将暂存的children恢复";
			...
			getState: () =>
				({
					runtime: {
						turn: {
							getCurrentSession: () =>
								createHistorySession(1, { userInput: longInput }),
						},
					},
					view: { getTuiInstance: () => tui },
				}) as never,
	...
	// The renderer must not throw — every line written to the terminal must
	// fit within the configured column count.
	for (const chunk of terminal.writes) {
		for (const line of chunk.split(/\r?\n/)) {
			assert.ok(
				visibleWidth(line) <= terminal.columns,
				`rendered line exceeds terminal width: ${visibleWidth(line)} > ${terminal.columns}: ${JSON.stringify(line)}`,
			);
		}
	}
...
test("/history falls back to writeLine when no TUI is available", async () => {
	const outputs: string[] = [];
	await executeChatCommand("/history", createChatCommandDefinitions(), {
		getState: () =>
			({
	...
	assert.equal(outputs.length, 1);
	assert.match(outputs[0] ?? "", /Turn 1 \[completed\]/);
	assert.match(outputs[0] ?? "", /Turn 2 \[completed\]/);
});
...
class FakeTerminal implements Terminal {
	...
	queryCellSize(): Promise<{ rows: number; cols: number } | null> {
		return Promise.resolve(null);
	}
	queryBackgroundColor(): Promise<string | null> {
		return Promise.resolve(null);
	}
	queryTerminalColorScheme(): Promise<"dark" | "light" | null> {
		return Promise.resolve(null);
	}
}
...
class BaseTextComponent implements Component {
	...
	render(width: number): string[] {
		return this.lines.map((line) => line.padEnd(width).slice(0, width));
	}
...
/**
 * Extract the visible text lines from the raw escape sequences written to the
 * fake terminal. Strips ANSI cursor movement and colour codes so tests can
...
function renderedLines(writes: string[]): string[] {
	...
	for (const segment of segments) {
		const stripped = segment.replace(/\x1B\[[0-9;?]*[ -/]*[@-~]/g, "");
		const cleaned = stripped.replace(/[\x00-\x08\x0B-\x1F\x7F]/g, "");
