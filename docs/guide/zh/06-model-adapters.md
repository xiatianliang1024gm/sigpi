...
`ModelProvider`（`src/model/provider.ts`）只有一个方法：
...
循环需要的一切都在这个接口之后。换 provider（或在测试里 mock 它）就是换一个对象。接口在模型模块里定义和构造，调用方通过 `createModelProvider(config, logger)` 获取，从不直接引用具体类。`OpenAICompatibleProvider` 是具体实现。
